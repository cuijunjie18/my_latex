...
\begin{tikzpicture}
    ...
    \draw [->] (a) node at (3,11) {loop0}arc(160:-160:-1);
    \draw [->] (b) node at (7,11) {loop1} arc(160:-160:-1);
    \draw [->] (c) node at (11,11) {loop2} arc(160:-160:-1);
...
in3 : \text{cnt\_baud = BAUD\_MAXN \&\& cnt\_bit = 4'd7} \\
loop0 : \text{valid = 0} \\
loop1 : \text{cnt\_baud != BAUD\_MAXN} \\
loop2 : \text{cnt\_baud != BAUD\_MAXN || cnt\_bit != 4'd7} \\
back : 等价态
\newpage
\end{document}